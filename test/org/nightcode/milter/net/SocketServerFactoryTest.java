...
public class SocketServerFactoryTest {
  ...
  private static final SocketAddress TEST_ADDRESS = new DomainSocketAddress("target/socket");

  @Test public void testSocket() throws InterruptedException, ExecutionException, TimeoutException {
    ...
    if ("Mac OS X".equalsIgnoreCase(System.getProperty("os.name"))) {
      workerGroup  = new KQueueEventLoopGroup(1, namedThreadFactory("jmilter-test-worker-kqueue"));
      channelClass = KQueueDomainSocketChannel.class;
    } else {
      workerGroup  = new EpollEventLoopGroup(1, namedThreadFactory("jmilter-test-worker-epoll"));
      channelClass = EpollDomainSocketChannel.class;
    }

    ServerFactory<SocketAddress> serverFactory = ServerFactory.socketFactory(TEST_ADDRESS);

    try (MilterGatewayManager<SocketAddress> gatewayManager = new MilterGatewayManager<>(serverFactory, milterHandler)) {
